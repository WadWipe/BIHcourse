...
age_seq <- c(49,59,69,79,999)
names_age_seq <- c("<50","50-59","60-69","70-79",">80")
table_dt <- 
  my_data %>% 
  mutate(Age = cut(AGE, breaks=c(0,age_seq),labels=names_age_seq)) %>%
  group_by(Age) %>%
  summarise(N = n()) %>%
  mutate(Percents = paste("(",percent(N/sum(N)),")", sep=""))
table_dt
######
#now for Table 2 
...
table_dt<-
  ...
  group_by(my_data$HEP, DEAD_verb)%>%
  summarise(N =n()) %>%
  mutate(Percents = paste("(",percent(N/sum(N)),")", sep=""))
